...
for mouseNum = 1:length(folders)
    ...
    escapeIndices = DataAll{3}{mouseNum,assayNum}(2,:); %get the behavior variable. (1=approach,2=eat)
...
for mouseNum = 1:length(folders)
    ...
    escapeIndices = DataAll{3}{mouseNum,assayNum}(2,:); %AGAIN -- set the behavior variable (1=approach, 2=eat)
    escapeIndices(1) = 0; escapeIndices(end) = 0;    
    temp = diff(escapeIndices); escapeFrameMS(:,1) = find(temp==1); escapeFrameMS(:,2) = find(temp==-1);
    clearvars temp
...
for mouseNum = 1:length(folders)
    ...
    yBound = [.4 1];
        ...
        title('5-fold logistic regression - predict eat walnut (n=4)')
