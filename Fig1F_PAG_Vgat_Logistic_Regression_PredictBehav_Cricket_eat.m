...
folders{1,1} = 'E:\PAG_VGAT_hunt\8_24_2020\635';
folders{2,1} = 'E:\PAG_VGAT_hunt\8_24_2020\636';
folders{3,1} = 'E:\PAG_VGAT_hunt\8_24_2020\637';
folders{4,1} = 'E:\PAG_VGAT_hunt\8_24_2020\641';
...
sampleDur = 5; sampleDur = round(sampleDur .* 7.5); %how long to train on for each behavior.
assayNum = 1; %only assay -- hunt cricket

    for mouseNum = 1:size(folders,1)
        cd(folders{mouseNum,assayNum})
        %load('Seg_3secPreBehav.mat','behavSegAll');
        load('good_neurons.mat')
        %load('Seg.mat','behavSegAll');         
        
        %if length(behavSegAll) < 4
        %    behavSegAll{4} = [];
        %end
...
for mouseNum = 1:length(folders)
    ...
    escapeIndices = DataAll{3}{mouseNum,assayNum}(2,:); %get the behavior variable. (1=approach cricket,2=eat cricket)
    ...
    %remove escapes that happen within 5 seconds of others
        for behavNum = 1:size(escapeFrameMS,1)-1
                escapeDiffRemoveIdx = diff(escapeFrameMS(:,1));
                escapeDiffRemoveIdx = find(escapeDiffRemoveIdx < (7.5*5)); escapeDiffRemoveIdx = escapeDiffRemoveIdx + 1;
...
for mouseNum = 1:length(folders)
    ...
    %fix this -- on 11/20 Friday, starting here.
    for escapeNum = 1:size(escapeFrameMS,1)
        %if (escapeFrameMS(escapeNum,1)-(max(timeLags))) > 0
        %    inPoint = escapeFrameMS(escapeNum,1)-(max(timeLags));
        %else
            %inPoint = 1;
        %end
...
for mouseNum = 1:length(folders)
    ...
    if numBehavs(mouseNum) < 3 %make sure there are at least x escape attempts per mouse
    ...
    yBound = [.4 1];
        ...
        title('5-fold logistic regression - predict eat cricket (n=4)')
